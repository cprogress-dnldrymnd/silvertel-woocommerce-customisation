 ...
 * Description: Custom modifications for WooCommerce, including a dynamic CSV import interceptor to sideload PDF URLs from specified meta fields into the Media Library.
 * Version: 1.1.0
 ...
 * * Encapsulates custom WooCommerce modifications, including dynamic PDF sideloading during CSV imports
...
class Silvertell_Woocommerce_Customisation
{
    ...
    {
        register_setting('silvertell_pdf_importer_group', 'silvertell_pdf_meta_keys');
    }
    ...
    public function render_settings_page()
    {
        ...
        $current_keys = get_option('silvertell_pdf_meta_keys', '_manual');
?>
        <div class="wrap">
            <h1>WooCommerce PDF Importer Settings</h1>
            <p>Specify which meta keys the importer should scan for PDF URLs. The system will automatically download these files to the Media Library and replace the URL with the integer Attachment ID.</p>

            <form method="post" action="options.php">
                <?php settings_fields('silvertell_pdf_importer_group'); ?>

                <table class="form-table" role="presentation">
                    <tbody>
                        <tr>
                            <th scope="row"><label for="silvertell_pdf_meta_keys">Target Meta Keys</label></th>
                            <td>
                                <input type="text" id="silvertell_pdf_meta_keys" name="silvertell_pdf_meta_keys" value="<?php echo esc_attr($current_keys); ?>" class="regular-text" style="width: 100%; max-width: 500px;">
                                <p class="description">Enter comma-separated meta keys. Example: <code>_manual, _datasheet, _brochure</code></p>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<?php
    ...
    public function intercept_meta_for_sideload($product, $data)
    {
        ...
        $meta_keys_string = get_option('silvertell_pdf_meta_keys', '_manual');
