...
class Silvertell_Woocommerce_Customisation {
	...
	public function render_custom_product_data_panels() {
		...
		// Note: The HTML name is intentionally aliased here to prevent POST collision, while preserving the _manual database structure via JS sync
		$this->render_single_file_upload_field( '_manual', __( 'Eval Board Manual', 'silvertell-wc-customisation' ), 'Upload the primary evaluation board manual.', '_manual_eval_dummy' );
		echo '</div>';
	}

	/**
	 * Renders a standalone Media Library file upload field.
	 * Includes a JS sync-class to dynamically mirror data across tabs.
	 */
	private function render_single_file_upload_field( $meta_key, $label, $description = '', $html_name = null ) {
		...
		$sync_class = 'dd-sync-' . esc_attr( $meta_key );
		...
		echo '<input type="hidden" name="' . esc_attr( $html_name ) . '" id="' . esc_attr( $html_name ) . '" class="dd-file-input ' . $sync_class . '" data-sync="' . $sync_class . '" value="' . esc_attr( $raw_val ) . '" />';
	...
	public function save_custom_product_data( $post_id ) {
		...
		// 2. Save the Unified `_manual` Field
		// The Eval Dummy field is completely ignored because JS perfectly synced its value into `_manual` directly in the browser before saving.
